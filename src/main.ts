...
function getFrontmatterTags(tagsValue: unknown): string[] {
	if (Array.isArray(tagsValue)) {
		return tagsValue
			.map((tag) => normalizeSingleTag(String(tag)))
			.filter((tag) => tag.length > 0);
	}
...
function buildExcerpt(raw: string): string {
	...
	const source = firstContentLine?.trim() ?? withoutFrontmatter;
...
class InboxProcessorView extends ItemView {
	...
	constructor(leaf: WorkspaceLeaf, private readonly plugin: InboxProcessorPlugin) {
		super(leaf);
	}
	...
	private resolveParentFile(input: string, currentFile: TFile): TFile | undefined {
		...
		return this.parentSuggestions.find((candidate) => {
			...
			return (
				...
				candidate.basename.toLowerCase() === normalized
			);
		});
	}
